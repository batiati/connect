...
Requires TigerBeetle version 0.16.57 or later.
		...
		Fields(
			...
			service.NewStringField(fieldProgressCache).
				Description("A https://docs.redpanda.com/redpanda-connect/components/caches/about[cache resource^] "+
					"to use for storing the current progress. This allows Redpanda Connect "+
					"to continue from the latest delivered timestamp upon restart."),
...
func (input *tigerbeetleInput) consume(ctx context.Context) error {
	...
	for {
		...
		case results := <-input.producerChan:
			...
			// Waits until the batch is acknowledged or the job is aborted.
			select {
